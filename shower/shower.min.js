 ...
 */
!function(global) {
	var undef, DECL_STATES = {NOT_RESOLVED: "NOT_RESOLVED", IN_RESOLVING: "IN_RESOLVING", RESOLVED: "RESOLVED"},
		create = function() {
			var curOptions = {trackCircularDependencies: !0, allowMultipleDeclarations: !0}, modulesStorage = {},
				waitForNextTick = !1, pendingRequires = [], define = function(name, deps, declFn) {
					declFn || (declFn = deps, deps = []);
					var module = modulesStorage[name];
					module || (module = modulesStorage[name] = {name: name, decl: undef}), module.decl = {
						name: name,
						prev: module.decl,
						fn: declFn,
					...
					var require, requiresToProcess = pendingRequires, i = 0;
					...
					var isProvided = !1, deps = decl.prev ? decl.deps.concat([decl.prev]) : decl.deps;
					decl.state = DECL_STATES.IN_RESOLVING, requireDeps(decl, deps, path, function(depDeclsExports, error) {
						...
						}), void decl.fn.apply({name: decl.name, deps: decl.deps, global: global}, depDeclsExports))
					})
			...
			for (var pathDecl, i = 0; pathDecl = path[i++];)if (decl === pathDecl)return !0;
			return !1
		}, nextTick = function() {
			var fns = [], enqueueFn = function(fn) {
			...
			if (global.postMessage && !global.opera) {
				...
				if (isPostMessageAsync) {
					var msg = "__modules" + +new Date, onMessage = function(e) {
						e.data === msg && (e.stopPropagation && e.stopPropagation(), callFns())
					};
					return global.addEventListener ? global.addEventListener("message", onMessage, !0) : global.attachEvent("onmessage", onMessage), function(fn) {
			...
			if ("onreadystatechange" in doc.createElement("script")) {
				var head = doc.getElementsByTagName("head")[0], createScript = function() {
	...
	function initialize() {
		global.shower.modules.require("shower.defaultOptions", function(defaultOptions) {
			var hasOptions = global.hasOwnProperty("showerOptions"), options = global.shower.options,
				containerSelector = options.shower_selector || defaultOptions.container_selector,
				element = document.querySelector(containerSelector), getDataAttr = getData.bind(this, element),
			...
			("false" !== getDataAttr("auto-init") || hasOptions && autoInit) && (hasOptions || dataAttrsOptions.forEach(function(name) {
				...
				sh.init({container: element, options: options})
			}))
		})
	}
	...
	extend(EventEmitter.prototype, {
		on: function(types, callback, context, priority) {
			...
			if (priority = priority || 0, "string" == typeof types) this._addListener(types, callback, context, priority); else for (var i = 0, l = types.length; i < l; i++)this._addListener(types[i], callback, context, priority);
			return this
		}, off: function(types, callback, context, priority) {
			if (priority = priority || 0, "string" == typeof types) this._removeListener(types, callback, context, priority); else for (var i = 0, l = types.length; i < l; i++)this._removeListener(types[i], callback, context, priority);
			return this
			...
			var event = eventObject, listeners = this._listeners;
			...
			var data = {target: target, type: type};
			...
			var listener = {callback: callback, context: context, priority: priority};
	...
	extend(EventGroup.prototype, {
		on: function(types, callback, context) {
			if (Array.isArray(types))for (var i = 0, k = types.length; i < k; i++)this._listeners.push(types[i], callback, context); else this._listeners.push(types, callback, context);
			return this.events.on(types, callback, context), this
		}, off: function(types, callback, context) {
			if (Array.isArray(types))for (var i = 0, k = types.length; i < k; i++)this._removeListener(types[i], callback, context); else this._removeListener(types, callback, context);
			return this
		}, offAll: function() {
	...
	function Plugins(showerGlobal) {
		this.events = new EventEmitter({context: this}), this._showerGlobal = showerGlobal, this._showerInstances = showerGlobal.getInited(), this._plugins = {}, this._instances = [], showerGlobal.events.on("init", this._onShowerInit, this)
	}
	...
	extend(Plugins.prototype, {
		destroy: function() {
			...
			return this._requireAndAdd({name: name, options: options}), this
		}, remove: function(name) {
			if (!this._plugins.hasOwnProperty(name))throw new Error("Plugin " + name + " not found.");
			return delete this._plugins[name], this.events.emit("remove", {name: name}), this
		}, get: function(name, shower) {
			var pluginInstance, plugin = this._plugins[name];
			if (plugin && shower)for (var i = 0, l = this._instances.length; i < l; i++) {
			...
			}, this), this.events.emit("add", {name: plugin.name})
			...
			this._instances.push({shower: shower, plugin: plugin, instance: new plugin.class(shower, options)})
	...
	sh.events = new EventEmitter({context: sh}), sh.plugins = new Plugins(sh), sh.events.on("notify", function(e) {
	...
	inherit(Options, Store, {
		set: function(name, value) {
			...
			return changed.length && this.events.emit("set", {items: changed}), this
		}, unset: function(name) {
			return Options.super.unset(this, name), this.events.emit("unset", {name: name}), this
		}, getMonitor: function() {
	...
	extend(Monitor.prototype, {
		destroy: function() {
			...
			var handler = {callback: callback, context: context};
			...
			var fieldsHanders = this._fieldsHanders[field], handler = fieldsHanders.filter(function(hander) {
	...
	function Shower(container, options) {
		options = options || {}, this.events = new EventEmitter({context: this}), this.options = new Options({}, defaultShowerOptions, options);
	...
	extend(Shower.prototype, {
		destroy: function() {
			...
			if (Array.isArray.call(null, slide))for (var i = 0, k = slide.length; i < k; i++)this._addSlide(slide[i]); else this._addSlide(slide);
			return this
		}, remove: function(slide) {
			var slidePosition;
			if ("number" == typeof slide) slidePosition = slide; else {
				if (this._slides.indexOf(slide) == -1)throw new Error("Slide not found");
				slidePosition = this._slides.indexOf(slide)
			}
			return slide = this._slides.splice(slidePosition, 1)[0], this.events.emit("slideremove", {slide: slide}), slide.destroy(), this
		}, get: function(index) {
			...
			slide.state.set("index", this._slides.length), this._slides.push(slide), this.events.emit("slideadd", {slide: slide})
	...
	extend(Container.prototype, {
		init: function() {
			var bodyClassList = document.body.classList, showerOptions = this._shower.options,
			...
			var elementClassList = document.body.classList, showerOptions = this._shower.options;
	...
	function Location(shower) {
		...
		this.state = new SessionStore(sessionStoreKey, {isSlideMode: !1}), this._showerListeners = null, this._playerListeners = null, this._documentTitle = document.title, this._popStateProcess = null, this._setupListeners(), this._init()
	}
	...
	extend(Location.prototype, {
		destroy: function() {
			...
			var slideInfo, shower = this._shower, currentSlideId = window.location.hash.substr(1),
			...
			for (var slide, index, slides = this._shower.getSlides(), i = slides.length - 1; i >= 0; i--)if (slides[i].getId() === slideId) {
			...
			var slideInfo, shower = this._shower, slideId = window.location.hash.substr(1),
			...
			var title = document.title, isSlideMode = this._isSlideMode(),
	...
	extend(Player.prototype, {
		init: function() {
			...
			var slidesCount = this._shower.getSlidesCount(), currentSlide = this._currentSlide;
			...
			var slide = e.get("slide"), slideNumber = this._shower.getSlideIndex(slide);
			this.go(slideNumber)
		}, _onKeyDown: function(e) {
			if (this._shower.isHotkeysEnabled() && !/^(?:button|input|select|textarea)$/i.test(e.target.tagName)) {
				this.events.emit("keydown", {event: e});
	...
	extend(Slide.prototype, {
		init: function() {
			this.layout = "string" == typeof this._content ? new slideLayoutFactory.createLayout({content: this._content}) : new Layout(this._content, this.options), this.layout.setParent(this),
			...
			return this.state.set("visited", ++visited), this.events.emit("activate", {slide: this}), this
		}, deactivate: function() {
	...
	extend(Layout.prototype, {
		init: function() {
			...
			this._parent != parent && (this._clearListeners(), this._parent = parent, this._parent && this._setupListeners(), this.events.emit("parentchange", {parent: parent}))
		}, getParent: function() {
			return this._parent
		}, setParentElement: function(parentElement) {
			parentElement != this._parentElement && (this._parentElement = parentElement, parentElement.appendChild(this._element), this.events.emit("parentelementchange", {parentElement: parentElement}))
	...
	extend(layoutFactory, {
		createLayout: function(parameters) {
			...
			var element = layoutFactory._createElement(extend({content: "", contentType: "slide"}, parameters));
	...
	function Next(shower, options) {
		options = options || {}, this.events = new EventEmitter({context: this}), this._shower = shower, this._elementsSelector = options.selector || DEFAULT_SELECTOR, this._elements = [], this._innerComplete = 0, this._setupListeners(), this._shower.player.getCurrentSlideIndex() != -1 && this._onSlideActivate()
	}

	var TIMER_PLUGIN_NAME = "shower-timer", DEFAULT_SELECTOR = ".next";
	extend(Next.prototype, {
		destroy: function() {
			...
			if (!plugin) {globalShower.plugins.get(TIMER_PLUGIN_NAME, this._shower)}
			...
			var slideLayout = this._shower.player.getCurrentSlide().layout, slideElement = slideLayout.getElement();
			return Array.prototype.slice.call(slideElement.querySelectorAll(this._elementsSelector))
		}, _onNext: function(e) {
			var elementsLength = this._elements.length, isSlideMode = this._shower.container.isSlideMode();
	...
	extend(Timer.prototype, {
		destroy: function() {
			...
			var events = this.events, nextPlugin = (this._shower, this._nextPlugin);
	...
	extend(Touch.prototype, {
		destroy: function() {
			...
			var x, shower = this._shower, isSlideMode = shower.container.isSlideMode(), element = e.target,
			...
			for (var slides = this._shower.getSlides(), result = null, i = 0, k = slides.length; i < k; i++)if (element.id === slides[i].getId()) {
